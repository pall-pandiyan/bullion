(function($) {
	...
	"use strict";
	var award_carousel_js = function($scope, $) {
		
		// three-item-carousel
		var design_one = $('.three-item-carousel'); 
        if(design_one.length){
            var slider_attr = $('#yt3-slider').data('slider');
	...
	$(window).on('elementor/frontend/init', function () {
            elementorFrontend.hooks.addAction('frontend/element_ready/bullion_bullion_awards.default', award_carousel_js);
    });	
